...
type Detector = new (options?: { formats?: string[] }) => { detect: (source: HTMLVideoElement) => Promise<Array<{ rawValue?: string }>> };
...
export function VerifySearchClient({ defaultValue, error }: { defaultValue: string; error: boolean }) {
  ...
  async function startScanner() {
    ...
    if (!DetectorClass || !navigator.mediaDevices?.getUserMedia) {
      setScanning(true);
      setMessage("Scanner tidak didukung browser ini. Masukkan nomor secara manual.");
      return;
    }
    try {
      ...
      setScanning(true);
      setMessage("Arahkan kamera ke QR atau barcode dokumen LHU.");
      ...
      const scan = async () => {
        ...
        try {
          ...
          if (result) {
            ...
            setValue(result);
            setMessage("Nomor berhasil dibaca. Tekan Verifikasi.");
            return;
          }
        } catch {
          setMessage("Kamera aktif. Arahkan ke barcode yang terlihat jelas.");
        }
        frameRef.current = requestAnimationFrame(scan);
      };
      frameRef.current = requestAnimationFrame(scan);
      ...
      setScanning(true);
      setMessage("Kamera tidak dapat dibuka. Periksa izin kamera atau masukkan nomor manual.");
    }
  }
